...
int main(int argc, char* argv[]){
	...
	while(convergence){
		...
		// 1. use blocking send/recv 
		// 2. remember master/last process only need to send/recv one row
		if(myid == 0){
			MPI_Send(myJacobi[block-1], SIZE, MPI_FLOAT, myid+1, 17, MPI_COMM_WORLD);
			MPI_Recv(myJacobi[block], SIZE, MPI_FLOAT, myid+1, 23, MPI_COMM_WORLD, &status);
